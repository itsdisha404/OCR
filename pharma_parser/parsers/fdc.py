"""
fdc.py — FDC Limited
GSTIN: 07AAACF0253H1Z2
Digital PDF, 4 pages. Has DS/DN/F category column. PTR column before qty.
Products: Electral, Enerzal, Zifi, Vitcofol etc.
"""
...
class Parser(GenericParser):
    ...
    def _find_columns(self, header_row: list) -> dict:
        ...
        for i, cell in enumerate(header_row or []):
            ...
            if "product" in h or "desc" in h or "goods" in h:
                col["product"] = i
            elif "hsn" in h:
                col["hsn"] = i
            elif "batch" in h:
    ...
    def _parse_row(self, row, col, sr):
        ...
        return {
            ...
            "cgst_rate": parse_amount(g("cgst")) if parse_amount(g("cgst")) <= 50 else 2.5,
            "cgst_amount": parse_amount(g("cgst")) if parse_amount(g("cgst")) > 50 else 0.0,
            "sgst_rate": parse_amount(g("sgst")) if parse_amount(g("sgst")) <= 50 else 2.5,
            "sgst_amount": parse_amount(g("sgst")) if parse_amount(g("sgst")) > 50 else 0.0,
            "igst_rate": 0.0,
            "igst_amount": 0.0,
